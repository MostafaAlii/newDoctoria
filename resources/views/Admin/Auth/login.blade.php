...
@section('title'){{helperTrans('admin.login')}} @endsection
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="card mt-4">
                    ...
                    <div class="card-body p-4">
                        <div class="text-center mt-2">
                            <h5 class="text-primary">{{helperTrans('admin.welcome back')}} !</h5>
                            <p class="text-muted">Log in to continue to
                                {{setting()->app_name}}.</p>
                        </div>
                        <div class="p-2 mt-4">
                            <form action="{{route('admin.postLogin')}}" method="post" enctype="multipart/form" id="Form">
                                @csrf
                                <div class="mb-3">
                                    <label for="email" class="form-label">{{helperTrans('admin.Email')}}</label>
...
{{--                                    </div>--}}
                                    <label class="form-label" for="password-input"> {{helperTrans('admin.Password')}}</label>
...
{{--                                </div>--}}
                                <div class="mt-4">
                                    <button class="btn btn-success w-100" id="loginButton" type="submit">{{helperTrans('admin.Log in')}}</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
        ...
        $("form#Form").submit(function (e) {
            ...
            $.ajax({
                ...
                complete: function () {
                    $('#loginButton').html(`<i id="lockId" class="fa fa-lock" style="margin-left: 6px"></i> {{helperTrans('admin.Sign In')}}`).attr('disabled', false);
                ...
                success: function (data) {
                    ...
                    if (data == 200) {
                        ...
                        window.setTimeout(function () {
                            toastr.error('{{helperTrans('admin.There is an error in the password or your account has been suspended')}}')
                        }, 750);
                    }
