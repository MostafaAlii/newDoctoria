...
@section('content')
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <h5 class="card-title mb-0 flex-grow-1">{{helperTrans('admin.Branches For Pharmacy:')}}{{$pharmacy->name}}</h5>

                <div>
                    <button id="addBtn-p" data-id="{{$pharmacy->id}}" class="btn btn-primary"> {{helperTrans('admin.Add Branch')}}</button>
                </div>
...
@section('js')
    ...
    @include('Admin.layouts.inc.ajax',['url'=>'pharmacy_branches'])

    <script>
        $(document).on('click', '#addBtn-p', function () {
            var pharmacy_id=$(this).attr('data-id');
            var route="{{route('pharmacy_branches.create')}}?pharmacy_id="+pharmacy_id;
