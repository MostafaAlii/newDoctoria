...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('diagnoses.update',$row->id)}}">
    @csrf
    @method('PUT')
    <div class="row g-4">
        ...
        @foreach(languages() as $index=>$language)
            ...
            <div class="d-flex flex-column mb-7 fv-row col-sm-6">
                ...
                <input id="diagnosis_{{$language->abbreviation}}" required type="text" class="form-control form-control-solid"
                       placeholder="" name="diagnosis[{{$language->abbreviation}}]" value="{{$row->getTranslation('diagnosis', $language->abbreviation)}}"/>
            </div>
