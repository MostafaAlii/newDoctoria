...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('providers.update',$row->id)}}">
    @csrf
    @method('PUT')
    <div class="row g-4">
    ...
    @foreach(languages() as $index=>$language)
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-12">
            ...
            <select name="provider_type_id" id="nationality_id" class="form-control">
                ...
                @foreach($provider_types as $provider_type)
                    <option value="{{$provider_type->id}}" {{$provider_type->id == $row->provider_type_id ? 'selected' : ''}}>{{$provider_type->name}}</option>
                @endforeach
        ...
        @foreach(languages() as $index=>$language)
            ...
            <div class="col-sm-6 pb-3 p-2">
                ...
                <textarea name="about_us[{{$language->abbreviation}}]" id="about_us_{{$language->abbreviation}}" class="form-control " rows="5" placeholder="">{{$row->getTranslation('about_us', $language->abbreviation)}}</textarea>
            </div>
