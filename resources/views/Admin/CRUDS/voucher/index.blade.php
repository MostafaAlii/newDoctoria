...
@section('content')
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <h5 class="card-title mb-0 flex-grow-1">{{helperTrans('admin.vouchers')}}</h5>
            @if(auth()->guard('admin')->user()->hasPermission('vouchers_create'))

                <div>
                    <button id="addBtn" class="btn btn-primary"> {{helperTrans('admin.Add Voucher')}}</button>
                </div>
        ...
        <div class="card-body">
            <table id="table" class="table table-bordered dt-responsive nowrap table-striped align-middle"
                ...
                <tr>
                    <th>#</th>
                    <th>{{helperTrans('admin.Code')}}</th>
                    <th>{{helperTrans('admin.Phone')}}</th>
                    <th>{{helperTrans('admin.Start At')}}</th>
                    <th>{{helperTrans('admin.End At')}}</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg mw-650px">
            ...
            <div class="modal-content" id="modalContent">
                ...
                <div class="modal-header">
                    <!--begin::Modal title-->
                    <h2><span id="operationType"></span> {{helperTrans('admin.Voucher')}} </h2>
                    <!--end::Modal title-->
                    <!--begin::Close-->
...
@section('js')
    ...
    @include('Admin.layouts.inc.ajax',['url'=>'vouchers'])
