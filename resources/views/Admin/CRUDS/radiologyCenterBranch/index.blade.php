...
@section('content')
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <h5 class="card-title mb-0 flex-grow-1">{{helperTrans('admin.Branches For Radiology Center:')}}{{$radiology_center->name}}</h5>

                <div>
                    <button id="addBtn-p" data-id="{{$radiology_center->id}}" class="btn btn-primary"> {{helperTrans('admin.Add Branch')}}</button>
                </div>
...
@section('js')
    ...
    @include('Admin.layouts.inc.ajax',['url'=>'radiology_center_branches'])

    <script>
        $(document).on('click', '#addBtn-p', function () {
            var radiology_center_id=$(this).attr('data-id');
            var route="{{route('radiology_center_branches.create')}}?radiology_center_id="+radiology_center_id;
