...
@section('title')
    {{helperTrans('admin.Doctor Branches Times')}}
@endsection
@section('css')
...
@section('content')
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <h5 class="card-title mb-0 flex-grow-1">{{helperTrans('admin.Times For Doctor Branches')}} {{$doctor->name}}</h5>
            <div>
                <button id="addNewRow" class="btn btn-primary"> {{helperTrans('admin.Add New Time')}}</button>
            </div>

        </div>
        <form id="form" enctype="multipart/form-data" method="POST" action="{{route('admin.update_doctor_branches_times',$doctor->id)}}">
            @csrf
            <div class="card-body" id="containerData">
                @foreach($doctorTimes as $doctorTime)
                    <div class="card" id="row_{{$doctorTime->id}}">
                        <div class="card-header text-center">
                            <span data-id="{{$doctorTime->id}}" class="text-center text-danger deleteRow" style="cursor: pointer"><i class="fa fa-trash"></i></span>
                        </div>
                        <div class="card-body">
                            <div class="row g-4">
                ...
                <div class="text-center mb-4">
                    <button form="form" type="submit" id="submit" class="btn btn-outline-primary">{{helperTrans('admin.Update')}}</button>
                </div>
            </div>
