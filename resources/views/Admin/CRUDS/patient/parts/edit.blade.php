...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('patients.update',$row->id)}}">
    ...
    <div class="row g-4">
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-6">
            ...
            <!--end::Label-->
            <input type="text" id="refer_code" class="form-control form-control-solid" placeholder=" " name="refer_code" value="{{$row->refer_code}}"/>
        </div>
            ...
            <select name="package_id" id="package_id" class="form-control">
                <option selected disabled>Select package</option>
                    @foreach($packages as $package)
                        <option value="{{$package->id}}" {{$package->id == $row->package_id ? 'selected' : ''}}>{{$package->name}}</option>
                    @endforeach
            </select>
        </div>
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-4">
            ...
            <select name="nationality_id" id="nationality_id" class="form-control">
                ...
                @foreach($nationalities as $nationality)
                    <option @if($row->nationality_id==$nationality->id) selected @endif value="{{$nationality->id}}">{{$nationality->name}}</option>
                @endforeach
