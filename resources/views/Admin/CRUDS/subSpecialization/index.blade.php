...
@section('title')
    {{helperTrans('admin.Sub Specialization')}}
@endsection
@section('css')
...
@section('content')
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <h5 class="card-title mb-0 flex-grow-1">{{helperTrans('admin.Sub Specializations For Specialization:')}} {{$parent->name}}</h5>


                <div>
                    <button id="addBtn-p" data-id="{{$parent->id}}" class="btn btn-primary"> {{helperTrans('admin.Add Sub Specialization')}}</button>
                </div>
        ...
        <div class="card-body">
            <table id="table" class="table table-bordered dt-responsive nowrap table-striped align-middle"
                ...
                <tr>
                    ...
                    <th>{{helperTrans('admin.color')}}</th>
                    <th>{{helperTrans('admin.icon')}}</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg mw-650px">
            ...
            <div class="modal-content" id="modalContent">
                ...
                <div class="modal-header">
                    <!--begin::Modal title-->
                    <h2><span id="operationType"></span> {{helperTrans('admin.Sub Specialization')}} </h2>
                    <!--end::Modal title-->
                    <!--begin::Close-->
...
@section('js')
    ...
    @include('Admin.layouts.inc.ajax',['url'=>'sub_specializations'])

    <script>
        $(document).on('click', '#addBtn-p', function () {
            var parent_id=$(this).attr('data-id');
            var route="{{route('sub_specializations.create')}}?parent_id="+parent_id;
